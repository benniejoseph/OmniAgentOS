import { getBearerToken, getMobileIdentityFromRequest, revokeMobileSession } from "@/lib/auth/mobile";
...
async function POSTHandler(request: Request) {
  ...
  const identity = token ? await getMobileIdentityFromRequest(request) : null;
  if (!token || !identity) return mobileError(401, "unauthorized", "A valid bearer token is required.");
