...
async function GETHandler(
  ...
  const authorized = await authorizeTrash(request, trashId, "restore_preview");
  if (authorized instanceof Response) return authorized;
  try {
    const result = await previewTrashRestoreService(
      createRequestMutationAppServiceCaller(request, authorized, {
        purpose: "trash.restore.preview",
        causationId: trashId,
      }),
      { trashId },
    );
...
async function POSTHandler(
  ...
  const { id: trashId } = await context.params;
  const authorized = await authorizeTrash(request, trashId, "restore");
  if (authorized instanceof Response) return authorized;
  let body: unknown;
  try { body = await parseJsonBody(request, 16_000); } catch (error) { return jsonBodyErrorResponse(error); }
  try {
    const result = await restoreTrashService(
      createRequestMutationAppServiceCaller(request, authorized, {
        purpose: "trash.restore",
        causationId: trashId,
      }),
      body as never,
    );
