...
import { reconcileSalesforceWorkspace } from "@/lib/customer-success/salesforce-sync";
...
async function POSTHandler(request: Request) {
  ...
  try {
    security = await authorizeRequest({
      ...
      resourceType: "salesforce_reconciliation",
      metadata: { operation: "read_only_reconciliation" },
    });
  } catch (error) {
  ...
  try {
    ...
    const result = await reconcileSalesforceWorkspace({
    ...
    return Response.json(
      {
        error: error instanceof SalesforceProviderError
          ...
          : "Salesforce reconciliation failed and can be retried safely.",
