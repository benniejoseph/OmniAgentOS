...
const assignmentLabels: Record<ModelAssignmentScope, { title: string; description: string }> = {
  ...
  code_builder: { title: "Code builder", description: "Forge application building and code verification" },
...
function McpConfiguration({ config, busy, onSave }: { config: McpExportConfiguration; busy: boolean; onSave: (config: McpConfigurationDraft) => Promise<unknown> }) {
  ...
  return <div className={clsx("mt-9 border-t border-line pt-7", styles.mcpSection)}>
    <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-md bg-primary/10 text-primary"><Network size={18} /></span><div><h3 className="text-base font-semibold">Asael MCP server</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-muted">Expose reviewed tools and optional resources at <code className="rounded bg-surface-raised px-1.5 py-0.5 font-mono">{config.endpointPath}</code>. Every tool execution remains governed.</p></div></div>
