...
export async function createDelegationTask(input: {
  ...
  return runWithDatabaseActorScope(
    task.tenantId,
    [task.ownerActorId],
    () => getSql().transaction(async (sql: DelegationSql) => {
...
export async function transitionDelegationTask(input: {
  ...
  requireDatabase();
  await ensureDatabaseSchema();
  const ownerActorId = requiredScopeActor(input.parentExecutionScope);
  return runWithDatabaseActorScope(
    input.tenantId,
    [ownerActorId],
    () => getSql().transaction(async (sql: DelegationSql) => {
...
function requiredScopeActor(scope: ExecutionScope) {
  const actorId = scope.initiatingActorId?.trim();
  if (!actorId) {
    throw new DelegationTaskConflictError(
      "Delegation task persistence requires an initiating actor.",
    );
  }
  return actorId;
}
