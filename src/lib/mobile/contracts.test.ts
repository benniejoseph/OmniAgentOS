...
describe("native API contracts", () => {
  it("retains exactly the current and previous rollout versions", () => {
    expect(NATIVE_API_CURRENT_VERSION).toBe(5);
    expect(NATIVE_API_PREVIOUS_VERSION).toBe(4);
    expect(nativeOperationsForVersion(5)?.length).toBeGreaterThan(
      nativeOperationsForVersion(4)?.length || 0,
    );
    expect(nativeOperationsForVersion(6)).toBeUndefined();
    expect(nativeContractSchemas.NativeContractDiscovery.parse(
      nativeContractDiscovery(),
    ).supportedVersions).toEqual([5, 4]);
  });
  ...
  it("requires bootstrap to advertise the authoritative contract rollout", () => {
    ...
    const identity = {
      ...
      device: { id: "device-one", name: "Asael on iOS", platform: "ios", appVersion: "1.0.0", buildNumber: 2, clientContractVersion: 5 },
    };
    expect(nativeBootstrapResponseSchema.parse({
      ...
      client: {
        ...
        requiredContractVersion: 5,
        supportedContractVersions: [5, 4],
        status: "compatible",
        agentCatalogEnrollment: { state: "held", clientReady: true },
      },
      nativeClientPolicy: { schemaVersion: 1 },
    }).api.nativeContract.currentVersion).toBe(5);
  });
});
