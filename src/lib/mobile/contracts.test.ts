...
describe("native API contracts", () => {
  it("retains exactly the current and previous rollout versions", () => {
    expect(NATIVE_API_CURRENT_VERSION).toBe(4);
    expect(NATIVE_API_PREVIOUS_VERSION).toBe(3);
    expect(nativeOperationsForVersion(4)?.length).toBeGreaterThan(
      nativeOperationsForVersion(3)?.length || 0,
    );
    expect(nativeOperationsForVersion(5)).toBeUndefined();
    expect(nativeContractSchemas.NativeContractDiscovery.parse(
      nativeContractDiscovery(),
    ).supportedVersions).toEqual([4, 3]);
  });
  ...
  it("accepts current and previous device envelopes but rejects partial attestation", () => {
    ...
    expect(nativeLoginRequestSchema.safeParse({
      ...request,
      device: { ...request.device, clientContractVersion: 3 },
    }).success).toBe(true);
    expect(nativeLoginRequestSchema.safeParse({
      ...request,
  ...
  it("requires bootstrap to advertise the authoritative contract rollout", () => {
    ...
    const identity = {
      ...
      device: { id: "device-one", name: "Asael on iOS", platform: "ios", appVersion: "1.0.0", buildNumber: 2, clientContractVersion: 4 },
    };
