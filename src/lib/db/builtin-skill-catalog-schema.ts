...
/**
 * Immutable catalog snapshot owned by ordered migration v187.
 *
 * Do not update this list in place. A later catalog change needs a new ordered
 * migration so already-applied schema versions keep their original meaning.
 */
export const BUILTIN_SKILL_CATALOG_V2_IDS = Object.freeze([
  "core.research",
  "core.builder",
  "core.critic",
  "core.memory",
...
export const BUILTIN_SKILL_ASSIGNMENT_LIMIT_V2 = 8;

const builtInSkillSqlArray = BUILTIN_SKILL_CATALOG_V2_IDS
  .map((id) => `      '${id}'`)
  .join(",\n");

/** Runtime equivalent of ordered migration v187 for non-production setup. */
export const BUILTIN_SKILL_CATALOG_V2_SCHEMA_SQL = String.raw`
  DO $migration$
  BEGIN
  ...
  BEGIN
    ...
    IF EXISTS (
      ...
      WHERE agent.skill_ids IS NULL
        OR cardinality(agent.skill_ids) > ${BUILTIN_SKILL_ASSIGNMENT_LIMIT_V2}
        OR EXISTS (
          SELECT 1
    ...
    BEGIN
      ...
      IF NEW.skill_ids IS NULL
        OR cardinality(NEW.skill_ids) > ${BUILTIN_SKILL_ASSIGNMENT_LIMIT_V2}
        OR EXISTS (
          SELECT 1
...
export async function ensureBuiltinSkillCatalogV2(sql: MigrationSql) {
  await sql.query(BUILTIN_SKILL_CATALOG_V2_SCHEMA_SQL);
}
