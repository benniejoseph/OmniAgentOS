export const PERSONAL_CONTEXT_CONSENT_SCHEMA_SQL = `
  ...
  AS $function$
    SELECT COALESCE(
      ...
      AND candidate_notice_contract_id =
        'notice:personal-context-automatic'
  ...
  CREATE TABLE IF NOT EXISTS omni_personal_context_consents (
    ...
    notice_contract_id TEXT NOT NULL DEFAULT
      'notice:personal-context-automatic',
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS
    omni_personal_context_consents_active_idx
  ...
  BEGIN
    ...
    IF OLD.state IS DISTINCT FROM 'active'
      ...
      OR ROW(
        ...
        NEW.tenant_id,
        NEW.actor_id,
        NEW.consent_generation,
        NEW.contract_id,
        NEW.notice_contract_id,
        NEW.notice_contract_version,
        NEW.notice_sha256,
        NEW.activated_by_actor_id,
        NEW.created_at,
        NEW.activated_at
        ...
        OLD.contract_id,
        OLD.notice_contract_id,
        OLD.notice_contract_version,
        OLD.notice_sha256,
  ...
  REVOKE ALL ON FUNCTION omni_personal_context_consent_row_is_valid(
    ...
    BIGINT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, TIMESTAMPTZ,
    TIMESTAMPTZ
  ) FROM PUBLIC;
