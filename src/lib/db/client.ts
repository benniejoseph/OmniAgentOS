...
// Tenant-scoped client: every tagged-template call is wrapped in a short
// BEGIN / SET LOCAL omni.tenant_id / <query> / COMMIT transaction when a
// tenant context is active, ensuring RLS sees the correct tenant in all cases.
function createTenantScopedSqlClient(pg: AnyPg): SqlClient {
  function withTenant<T>(fn: (sql: AnyPg) => Promise<T>): Promise<T> {
    ...
    return pg.begin(async (tx: AnyPg) => {
      await tx`SELECT set_config('omni.tenant_id', ${tenantId}, true)`;
      return fn(tx);
    });
  }
  ...
  // Called only from within this file (never from external code).
  scoped.transaction = (queriesOrFn: unknown, _opts?: unknown) => {
    const tenantId = getDatabaseTenantContext();
    if (typeof queriesOrFn === "function") {
      return pg.begin(async (tx: AnyPg) => {
        if (tenantId) await tx`SELECT set_config('omni.tenant_id', ${tenantId}, true)`;
        const txScoped = createTenantScopedSqlClient(tx);
        return Promise.all(
          (queriesOrFn as (s: SqlClient) => unknown[])(txScoped) as Promise<unknown>[],
        );
      });
    }
    return pg.begin(async (tx: AnyPg) => {
      if (tenantId) await tx`SELECT set_config('omni.tenant_id', ${tenantId}, true)`;
      return Promise.all(queriesOrFn as Promise<unknown>[]);
    });
  };
