...
function schemaMigrations(): SchemaMigration[] {
  return [
    ...
    {
      ...databaseSchemaMigrations[47],
      up: ensureTenantMemoryPurposeEntitlements,
    },
    {
...
async function ensureTenantMemoryPurposeEntitlements(sql: SqlClient) {
  await sql`
    DO $migration$
    BEGIN
      IF current_user IS DISTINCT FROM (
...
async function ensureTenantActorMemoryPurposeConsents(sql: SqlClient) {
  await sql`
    DO $migration$
    BEGIN
      IF current_user IS DISTINCT FROM (
