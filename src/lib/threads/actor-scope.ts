...
/**
 * Keeps request-bound PostgreSQL thread reads canonical-first while treating
 ...
 */
export function threadActorReadOrder(
  actorId: string,
...
function safeText(value: string, max: number) {
  return String(redactSensitive(value)).trim().slice(0, max);
}
