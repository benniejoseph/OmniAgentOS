...
BEGIN
  ...
  IF latest_version IS DISTINCT FROM 189 OR (
    SELECT count(*)
    FROM public.omni_schema_version
    WHERE version = 189
      AND name = 'generated_artifact_persistence_v1'
      AND checksum = '4065c615c77bf4baf5921d5dcd468359ed8113bc49ba5291908bdfc3b9f36ebf'
  ) <> 1 THEN
    RAISE EXCEPTION 'Moltbook Agent connection predecessor is invalid'
      USING ERRCODE = '55000';
  END IF;
END
$migration$;
...
CREATE TABLE omni_moltbook_connections (
  id TEXT PRIMARY KEY,
...
CREATE TABLE omni_moltbook_activities (
  id TEXT PRIMARY KEY,
  tenant_id TEXT NOT NULL,
  owner_actor_id TEXT NOT NULL,
  agent_id TEXT NOT NULL,
...
CREATE INDEX omni_moltbook_activities_owner_cursor_idx
...
CREATE FUNCTION omni_reject_moltbook_receipt_mutation_v1()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = pg_catalog, public
