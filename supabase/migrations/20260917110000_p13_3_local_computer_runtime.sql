...
BEGIN
  ...
  IF latest_version IS DISTINCT FROM 178 OR (
    SELECT count(*)
    FROM public.omni_schema_version
    WHERE version = 178
      AND name = 'p13_1_macos_native_platform_v1'
      AND checksum = 'b5cca5f81cd6cef3861d541d04ad4d15468476c8e954d610255fd3c70a237262'
  ) <> 1 THEN
    RAISE EXCEPTION 'Local Computer Use predecessor is invalid'
      USING ERRCODE = '55000';
  END IF;
END
$migration$;

CREATE TABLE public.omni_local_computer_devices (
  schema_version SMALLINT NOT NULL DEFAULT 1,
  tenant_id TEXT NOT NULL,
  owner_actor_id TEXT NOT NULL,
  user_id TEXT NOT NULL,
  mobile_session_id TEXT NOT NULL REFERENCES public.omni_mobile_sessions(id)
  ...
  CONSTRAINT omni_local_computer_devices_row_check CHECK (COALESCE(
    ...
    AND platform = 'macos' AND native_contract_version >= 11
...
CREATE INDEX omni_local_computer_devices_online_idx
  ON public.omni_local_computer_devices (
    tenant_id, owner_actor_id, enabled, lease_expires_at DESC, device_id
  );

CREATE TABLE public.omni_local_computer_sessions (
  schema_version SMALLINT NOT NULL DEFAULT 1,
  id TEXT NOT NULL,
  tenant_id TEXT NOT NULL,
  owner_actor_id TEXT NOT NULL,
...
CREATE TABLE public.omni_local_computer_commands (
  ...
  ) REFERENCES public.omni_local_computer_sessions (tenant_id, id)
    ON UPDATE RESTRICT ON DELETE CASCADE,
  CONSTRAINT omni_local_computer_commands_row_check CHECK (COALESCE(
    ...
    AND action IN ('observe', 'list_apps', 'activate_app', 'press', 'click', 'type', 'key', 'scroll')
    AND input_sha256 ~ '^[0-9a-f]{64}$'
    AND state IN ('queued', 'claimed', 'completed', 'failed', 'canceled', 'expired', 'consumed')
    AND claim_generation >= 0
    AND ((state = 'queued' AND claim_token_sha256 IS NULL) OR state <> 'queued')
    AND (claim_token_sha256 IS NULL OR claim_token_sha256 ~ '^[0-9a-f]{64}$')
    AND ((claimed_at IS NULL AND claim_expires_at IS NULL) OR (claimed_at IS NOT NULL AND claim_expires_at > claimed_at))
    AND (outcome IS NULL OR outcome IN ('succeeded', 'failed', 'canceled'))
    AND (result IS NULL OR (jsonb_typeof(result) = 'object' AND pg_column_size(result) <= 2097152))
...
CREATE INDEX omni_local_computer_commands_claim_idx
  ON public.omni_local_computer_commands (
    tenant_id, owner_actor_id, device_id, state, created_at, id
  );
CREATE INDEX omni_local_computer_commands_expiry_idx
  ON public.omni_local_computer_commands (expires_at, state);

ALTER TABLE public.omni_local_computer_devices ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.omni_local_computer_devices FORCE ROW LEVEL SECURITY;
ALTER TABLE public.omni_local_computer_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.omni_local_computer_sessions FORCE ROW LEVEL SECURITY;
ALTER TABLE public.omni_local_computer_commands ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.omni_local_computer_commands FORCE ROW LEVEL SECURITY;

CREATE POLICY omni_local_computer_devices_actor_scope
  ON public.omni_local_computer_devices FOR ALL TO PUBLIC
...
CREATE POLICY omni_local_computer_sessions_actor_scope
  ON public.omni_local_computer_sessions FOR ALL TO PUBLIC
  USING (public.omni_system_scope_enabled() OR public.omni_actor_scope_v1_allows(tenant_id, owner_actor_id) OR public.omni_actor_scope_v1_allows_canonical(tenant_id, owner_actor_id))
  WITH CHECK (public.omni_system_scope_enabled() OR public.omni_actor_scope_v1_allows(tenant_id, owner_actor_id) OR public.omni_actor_scope_v1_allows_canonical(tenant_id, owner_actor_id));
CREATE POLICY omni_local_computer_commands_actor_scope
  ON public.omni_local_computer_commands FOR ALL TO PUBLIC
  USING (public.omni_system_scope_enabled() OR public.omni_actor_scope_v1_allows(tenant_id, owner_actor_id) OR public.omni_actor_scope_v1_allows_canonical(tenant_id, owner_actor_id))
  WITH CHECK (public.omni_system_scope_enabled() OR public.omni_actor_scope_v1_allows(tenant_id, owner_actor_id) OR public.omni_actor_scope_v1_allows_canonical(tenant_id, owner_actor_id));

REVOKE ALL ON public.omni_local_computer_devices FROM PUBLIC;
REVOKE ALL ON public.omni_local_computer_sessions FROM PUBLIC;
REVOKE ALL ON public.omni_local_computer_commands FROM PUBLIC;

DO $grants$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'omni_runtime') THEN
    GRANT SELECT, INSERT, UPDATE ON public.omni_local_computer_devices TO omni_runtime;
    GRANT SELECT, INSERT, UPDATE ON public.omni_local_computer_sessions TO omni_runtime;
    GRANT SELECT, INSERT, UPDATE ON public.omni_local_computer_commands TO omni_runtime;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'omni_maintenance') THEN
    GRANT SELECT, INSERT, UPDATE, DELETE ON public.omni_local_computer_devices TO omni_maintenance;
    GRANT SELECT, INSERT, UPDATE, DELETE ON public.omni_local_computer_sessions TO omni_maintenance;
    GRANT SELECT, INSERT, UPDATE, DELETE ON public.omni_local_computer_commands TO omni_maintenance;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'omni_backup') THEN
    ...
    ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO omni_backup;
  END IF;
END
$grants$;
...
BEGIN
  IF (
    ...
    WHERE oid IN (
      'public.omni_local_computer_devices'::regclass,
      'public.omni_local_computer_sessions'::regclass,
      'public.omni_local_computer_commands'::regclass
...
VALUES (
  ...
  '68dcf5260d23b7f8e6eb2f14be43e8150a4ca747676b73e6ae27da6502f1fd68',
  clock_timestamp()
);
