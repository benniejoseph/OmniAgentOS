...
BEGIN
  ...
  IF latest_version IS DISTINCT FROM 171 OR (
    SELECT count(*) FROM public.omni_schema_version
    WHERE version = 171 AND name = 'app_builder_preview_deployments_v1'
      AND checksum = '22a1cc4db58ef6e999d0276af281a12d4876dc46964009ebd124d645327294ad'
  ) <> 1 THEN
    RAISE EXCEPTION 'App Builder production release predecessor is invalid' USING ERRCODE = '55000';
  END IF;
END
$migration$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'omni_runtime') THEN
    ...
    GRANT UPDATE (status, provider_project_id, provider_deployment_id, provider_state, deployment_url, logs, route_evidence, browser_evidence, failure_code, updated_at, released_at)
      ON public.omni_app_builder_releases TO omni_runtime;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'omni_maintenance') THEN
    GRANT SELECT, INSERT ON public.omni_app_builder_releases TO omni_maintenance;
    GRANT UPDATE (status, provider_project_id, provider_deployment_id, provider_state, deployment_url, logs, route_evidence, browser_evidence, failure_code, updated_at, released_at)
      ON public.omni_app_builder_releases TO omni_maintenance;
  END IF;
...
VALUES (172, 'app_builder_production_releases_v1', '7d4c51d2d01df3f14c2ccf263b8c2b029acaf1537db427d8b2353b4b3ea8fed0', clock_timestamp());
