...
task_source_dir="$task_flutter_dir/macos/CredentialBrokerV2"
task_signing_dir="${ASAEL_MACOS_LOCAL_SIGNING_DIR:-${HOME}/Library/Application Support/Asael/signing-v2}"
task_artifact_dir="${ASAEL_MACOS_CREDENTIAL_BROKER_DIR:-$task_signing_dir/credential-broker-v2}"
...
task_identity="${ASAEL_MACOS_SIGNING_IDENTITY:-${ASAEL_MACOS_LOCAL_SIGNING_IDENTITY:-Asael Private Code Signing 2026}}"
...
for task_command in xcrun swiftc lipo codesign shasum jq; do
  if ! command -v "$task_command" >/dev/null 2>&1; then
    echo "$task_command is required to provision macOS credential broker v2." >&2
    exit 1
  fi
done
...
signing_certificate_digest() {
  ...
  if [[ -z "${ASAEL_MACOS_SIGNING_IDENTITY:-}" && -f "$task_local_keychain" ]]; then
...
verify_artifact() {
  if [[ ! -d "$task_artifact_app" || ! -f "$task_manifest" || ! -x "$task_executable" ]]; then
    echo "Credential broker v2 is not provisioned at: $task_artifact_dir" >&2
    return 1
  fi
  if [[ "$(stat -f '%Lp' "$task_artifact_dir")" != "700" || "$(stat -f '%Lp' "$task_manifest")" != "600" ]]; then
    echo "Credential broker v2 directory/manifest permissions are not owner-only." >&2
    return 1
  fi
  ...
  jq -e \
    ...
    "$task_manifest" >/dev/null || {
      echo "Credential broker v2 verification failed. Never overwrite or re-sign v2; introduce v3." >&2
      return 1
    }
}
...
if [[ -e "$task_artifact_dir" ]]; then
  verify_artifact
  echo "Verified immutable credential broker v2: $task_artifact_app"
  exit 0
fi
if [[ "$task_verify_only" == true ]]; then
  echo "Run $0 once after installing the rotated signing identity and before packaging its first release." >&2
  exit 1
fi
...
if [[ -z "${ASAEL_MACOS_SIGNING_IDENTITY:-}" ]]; then
  if [[ ! -f "$task_local_keychain" || ! -f "$task_password_file" ]]; then
    echo "Install Asael's rotated private signing identity before provisioning broker v2." >&2
    exit 1
  fi
...
task_stage="$(mktemp -d "${TMPDIR:-/tmp}/asael-broker-v2.XXXXXX")"
...
jq -n \
  ...
  '{schema: 2, version: "2.0.0", build: "2", protocol: "credential-broker-v2",
    keychainService: "app.omniagent.omniagent.credential-broker.v2",
    initializationMarker: "asael.credential_broker_initialization_v2",
    requiresFreshSignIn: true, sourceDigest: $source,
...
verify_artifact
echo "Provisioned immutable credential broker v2: $task_artifact_app"
