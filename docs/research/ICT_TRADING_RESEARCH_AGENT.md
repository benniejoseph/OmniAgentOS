...
**Status:** Stage 0 foundation, Stage 1 immutable FRED history, and official BLS schedule enrichment implemented; observed-value vintages, non-BLS exact timestamps, impact windows, detectors, replay, calibration, and forward-shadow execution remain pending
...
The current UI exposes four progressively loaded views: Research desk, News impact lab, ICT + Quarterly, and Forecast journal. The chart library is loaded only when verified bars exist. The News Impact Lab can enqueue and track an owner-private background import for eight curated high-impact U.S. FRED releases from 2000 onward. The same job imports reviewed exact Eastern release times for CPI, PPI, Employment Situation, and JOLTS from the public BLS calendar into a separate immutable observation ledger. Other records retain date-only precision and every actual/consensus/prior field stays empty until an authoritative point-in-time value source is bound. Unavailable data and uncalibrated probabilities remain visibly unavailable; no sample prices, guessed timestamps, consensus values, or probabilities are rendered.
