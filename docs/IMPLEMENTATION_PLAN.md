...
56. P3.1 canonical user-private memory canary: migration v72 removes the dormant enrollment hold only for canonical actor-owned `user_private` rows; restrictive RLS binds every read and mutation to the exact transaction-local actor and canonical purpose; authenticated session/mobile memory create, list, search, inspect, correct, forget, export, and restore paths enter that scope; legacy rows remain on a separate compatibility lane; and each new bound row emits a metadata-only typed event. The authenticated Retrieval Plan API now merges the caller's independently scoped private-memory results into its context pack and suppresses tenant-wide trace persistence for every scoped request. Migration v74 repairs the daily-brief lineage column required by governed deletion invalidation. Migrations v75-v76 index immutable barrier manifests and remove redundant derived-row read scans only after proving the write-time/deferred deletion invariant; the production graph query fell from 15.46 seconds to 37.6 milliseconds, and the authenticated create/retrieve/redact/no-trace/forget canary passes. Agent-run context, private retrieval traces, agent-private/shared visibility, standing formation, and actor-aware graph projection remain pending, so P3.1 is not complete.
