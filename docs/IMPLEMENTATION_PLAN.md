...
> Plan status after item 64: the first safe delivery sequence is complete,
...
> explicit user-authored marker lane with deletion propagation and an
> actor-private operator review surface for ambiguous merges and reversals.
> Canonical user-private text evidence now projects the same explicit marker
> contract with revision/deletion propagation; only the representative
> production-like entity-resolution benchmark remains for P5.2.
