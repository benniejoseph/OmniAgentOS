...
- `POST /api/projects` and `PATCH /api/projects/:id` accept `Idempotency-Key` using 1–200 letters, numbers, dots, underscores, colons, or hyphens. When omitted, the server binds the mutation to the trusted request ID or a generated request correlation. Authenticated project creation is exactly replayable with a supplied key; reusing that key for different project content returns a conflict. These paths write their versioned, metadata-only domain event in the same Postgres transaction as the project row.
