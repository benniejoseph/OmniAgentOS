...
- `/api/runs` and `/api/runs/:id`: run history, events, and replay/continuation state. `GET /api/runs/:id/trajectory` returns bounded event receipts, checkpoint steps, structural verification, and parent/child fork lineage. `POST /api/runs/:id/fork` accepts an exact `checkpointId`, a 1–4,000 character correction, and an optional bounded `requestId`/matching `Idempotency-Key`; it creates and executes a new trace from the verified event prefix. The source trace is immutable, and the fork starts with a fresh execution scope, no inherited continuation, and no inherited approval.
