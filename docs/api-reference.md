...
- `/api/memory`, `/api/memory/graph`, `/api/knowledge`, `/api/ingest`, and `/api/retrieval/plan`. Authenticated session/mobile Retrieval Plan requests merge independently queried legacy memory and the current canonical user's private-memory cohort; because retrieval traces remain tenant-wide, any request entering private scope returns an ephemeral pack without persisting a trace. `GET /api/memory/:id?view=deletion-preview` requires memory-write authority and returns the exact descendant, retrieval-trace, graph-projection, and pending-run impact plus a receipt-manifest digest. `DELETE /api/memory/:id` requires that digest in `X-Asael-Deletion-Preview`; if the impact changed after review, deletion fails with `409` and requires a fresh preview. A successful response includes the permanent deletion receipt and exact invalidated-run counts.
