...
- `/api/settings/providers`, `/api/settings/models`, `/api/settings/assignments`, `/api/settings/api-keys`, and `/api/settings/mcp` manage redacted provider connections, lifecycle-aware model selection, service keys, and the actor-owned MCP export boundary. Credential plaintext is accepted only on create/rotation and is never returned after storage. `GET /api/settings/providers?ownerScope=readable` and `GET /api/settings?ownerScope=readable` opt provider metadata into the strictly bound canonical/current-email read; bare GETs stay exact. Successful responses acknowledge `requestReadContracts.providerConnections` as `readable_v1` or `exact_v1`; clients must require `readable_v1` before exposing provider-write controls. Provider rows include `manageable`; canonical connections are configuration-only and read-only, deployment fallbacks remain externally managed, and credential ciphertext/key IDs are never selected by the request reader. Authenticated model-catalog GETs may merge the same owners; `selectable` is true only for exact-owner identifiers accepted by the assignment API, while `displayModelId` is the safe UI projection of the persisted identifier. Authenticated service-key GETs select redacted columns and expose physical-owner-derived `manageable`. Provider credential opening, validation, rotation, updates/revocation, catalog refresh, runtime model resolution, assignments, key creation/revocation, Bearer verification, last-used updates, and the MCP export policy remain exact-owner operations.
