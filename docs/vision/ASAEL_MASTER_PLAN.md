...
gateway and Playwright gateway remain healthy. P9.4 replaces earned-autonomy
auto-approval with actor-private, short-lived, budgeted grants derived only from
a persisted workflow approval. Each grant binds the exact plan and digest,
approver and executing principal, domain/action/tool and contract digest, and
the digest of the complete reviewed static tool input. Consumption is atomic and
idempotent, replanning revokes the prior plan's grants, and changed or dynamic
inputs fall back to per-action approval. Risk-three and irreversible operations
are never grant-eligible. Migration 121 is installed with checksum
`74a51bdd87e583b6380533d34571455ba717829e390de8ef0806b2a818f37bd6`;
readback confirms forced RLS, restrictive actor policies, append-only claims,
and lifecycle-only grant updates. Ninety-two focused checks, affected lint,
TypeScript, and the 98-page build pass. Vercel deployment
`dpl_DtNitGtzr8sLfTuwgHZ2fhtYkJzQ` is Ready at exact revision
`c17aba2c40166f1f09ad9beee9f463ac6bba9dfe`; the web, worker/OpenAI gateway,
and Playwright gateway all return HTTP 200 health. P9.5 adds the versioned
server-pushed run-activity stream over
the durable owner-scoped run ledger. Safe Playwright actions retain both a
bounded screenshot and a separately redacted accessibility snapshot through the
existing opaque tenant/actor/run browser session; text entry, form fill, and
file upload retain neither. Raw image and snapshot bytes remain behind distinct
authenticated private/no-store routes and never enter Agent tool transcripts.
The viewer reconnects automatically, follows fresh frames without refresh, and
derives `Live` versus `Replay` from the persisted run status. No database
migration or Fly image change was required. Fifteen focused checks, affected
...
surfaces return HTTP 200. P9.7 adds actor-owned, opt-in profiles with exact
domain scope, lifecycle revisions, immutable run binding, revocation, and
metadata-only events. The web tier issues opaque five-minute profile authority
bound to the actor, run scope, profile revision, and domain digest. The isolated
Playwright gateway verifies that authority, restricts profile egress to the
consented domains, and AES-256-GCM seals the profile archive on a dedicated
encrypted Fly volume. A user may take over only a run paused on the exact
governed browser approval, privately type without persisting the value, then
hand execution back with fresh browser evidence and a durable resume wakeup.
Migrations 122 and 123 are installed; migration 123 repairs the permissive
tenant+actor RLS grant on the 12 affected A2A, Trash, approval, and browser
tables without broadening their predicate. The actor-scoped production canary
completed login, profile consent, readback, and revision-fenced revocation.
Vercel deployment `dpl_3BSWUNg6dC4wg8w573FBYokHHJnk` is Ready at exact
revision `972e60f7722c5ccec92f720d213f535d58f89716`; the Singapore browser
gateway is healthy at revision `80499c5619a673b7ea57cb4022163a846e0cd7d7`
with `persistentProfiles: true`, and the compatible worker/OpenAI gateway is
healthy. P9.8 installs the fixed `p9.8-browser-action-policy:1` classification
after validated input and actor/profile binding but before approval policy. Safe
observation, navigation, and known tab-management variants are read-only or
reversible risk-zero/one actions; exact reviewed workflow navigation is eligible
for the existing short-lived plan grant. Clicks, typing, forms, selections, key
presses, drags, dialogs, and unknown browser powers fail closed as irreversible
consequential effects, remain per-action approval-gated, and persist the existing
provider effect intent plus an honest post-action verification receipt. Arbitrary
code, upload/drop, and direct network powers remain risk three. Page labels,
accessibility text, model prose, and remote annotations cannot lower risk, and
profile revocation is revalidated before approval replay. Fifty-one focused
browser-policy, executor, workflow-grant, connector, and provider-effect checks,
affected lint, TypeScript, and the 99-page production build pass. No migration or
...
contract. P9.10 adds the immutable `asael-voice:1` profile, exact-result
streaming PCM synthesis through the allowlisted OpenAI gateway, progressive
browser playback, manual interruption, and VAD-driven barge-in over a fresh
owned transcription session. Speech lifecycle events retain only profile,
...
isolation and deterministic authorization then adds a separately reviewed
credential-provider interface, exact short-lived single-use scope, provider
ES256 signature verification, and an encrypted payment-only token boundary.
The scope binds the verified mandates, merchant, amount, currency, processor,
instrument reference, checkout hash, authorization, expiry, audience, and
32-byte nonce. Provider failure reconciles by exact request digest instead of
blindly retrying. Migration 126 installs actor-private forced-RLS grant and
append-only claim ledgers. Claiming is atomic, processor-specific, and destroys
the sealed token before dispatch; replay, redirect, changed scope, and changed
signatures fail closed. No API or Agent tool returns a scoped token, raw
credential, or signing material, and authorization alone still confers no
payment effect. Fifty-six focused contract, tamper, vault, readiness, service,
route, database-manifest, and migration checks pass with affected lint,
TypeScript, and the 104-page production build. Vercel deployment
`dpl_38CE96fmtXhJGEGV2LJpm6kcctkg` is Ready and the canonical alias serves exact
...
The v16 native release advances the generated contract to v16, retains
...
| P13.6 | Render rich Command answers, expose a truthful observable work log, and make Model/Thinking selection scope-correct. | Reuse the shared Talk controller/activity projection and Settings assignments; create a bounded native rich-response renderer, responsive activity entry point, and exact scoped model picker. | Interpret only supported Markdown blocks and safe web links; never treat model HTML or custom schemes as executable UI, never expose or persist private chain-of-thought, and never let a displayed catalog or stale selection grant execution authority. | Live Command output renders structured text, code, and tables with copy actions; View work shows only real plans, tools, approvals, evidence, context, agents, outcomes, and effective model metadata; Agent/**This Mac** changes reload the exact Settings scope and clear stale selection. |
| P13.7 | Add a stateful Asael companion and make voice capture states truthful. | Reuse the shared Talk controller and recorder; create one original segmented Lottie asset and an optional amplitude adapter. | The mascot grants no authority, reduced motion resolves to poster frames, capture/transcription remain explicit and interruptible, and a transcript is editable text rather than implicit command authority. | Ready, working, listening, transcribing, success, and attention states are visible and accessible; live voice capture shows actual levels and succeeds through retry without an idle or enabled-send gap. |

**Current status:** P13.1, P13.3, P13.4, P13.5, P13.6, and P13.7 are implemented for the
...
v28 with frozen v27 rollback compatibility; migration v205 is installed; and
owner-only Asael 1.22.0 build 34 is installed. P13.3 retains zero durable
...
P13.7 mascot communicates only observable app state, respects reduced motion,
and closes the voice startup/finalization and submission-enable gaps. The
latest P13.6 presentation patch preserves identifiers containing underscores
and displays the effective server-emitted Thinking intensity in the observable
model receipt without exposing private reasoning. Native v28 closes the scoped
selection follow-up: **This Mac** maps to `computer_use`; Forge, Sentinel,
Meridian, Mnemosyne, and Scout map to their exact specialist scopes; and
Atlas/custom/unassigned direct work maps to `main_agent`. Context changes clear
old Model/Thinking state, explicit choices force direct execution, durable
conflicts fail closed, and the server revalidates the exact Settings assignment.
The separate worker/OpenAI egress service remains protocol-compatible. Earlier
...
Migrations 179 and 180 install the forced-RLS device, session, command, exact-run,
and bounded-observation routing boundaries. Migration 181 is installed with
checksum `2d8bfc80ac843fe49ca79024022b873f5046a68822892ace7ff78d393025cf4d`;
its aggregate reports zero active remote profile, takeover, matching connector,
or tool authority while retaining read-only audit history. The Playwright product
and development source, package, CI, benchmark, visual-smoke, and deployment paths
are removed. Source migration 182 adds only `open_url` to the validated local
command-action constraint. Production validates it with checksum
`46a2975c9099d954bc7f7ff6aa537076f14f8dce274e53f33826a38471d1f5e4`.

The Flutter client keeps the native bearer and only its primary window engine may
claim an exact governed command. A separately signed Computer Use helper under
...
bounded observation, allowlisted Chrome URL delivery, app activation, press,
click, type, key, and scroll. Terminal applications, System Settings, secure
fields, Secure Event Input, shell, arbitrary AppleScript, general filesystem,
Apple Events, and unbounded input are refused. Risk-two effects remain approval-
gated, stale observations fail closed, one-turn visual/Accessibility data is
stripped from durable ledgers, and a ready/active menu-bar indicator provides
immediate stop.

A different separately signed frozen `AsaelCredentialBroker.app` keeps the
private-release Keychain owner stable and grants no Computer Use authority.
Broker v1.0.0 build 1 is universal, retains CDHash
`056b6bc5ce0709b430fd48dfb38f8d7d01b380e0`, validates its direct parent and
...
P13.7 is complete in the shared Flutter Talk surface. One original raster-free
Lottie composition carries six segmented states: ready, listening,
transcribing, working, success, and attention. The bounded widget is
repaint-isolated, semantically labelled, and freezes to a meaningful poster
frame when reduced motion is requested. The recorder exposes an optional live
amplitude stream, while startup, recording finalization, and transcription are
separate truthful UI states; prompt submission remains disabled across that
whole boundary. The installed live canary covered ready/working transitions,
real listening levels, no-clear-speech attention/retry, and successful spoken
transcription. Asael 1.21.0 build 32 is installed from
`Asael-1.21.0-32-macOS.dmg`, SHA-256
`2d7f80c11b59dad633db1adb99ea4ccf4baae17899ce7668f7d1961906330a33`;
the installed app CDHash is `4ca36fd3f7ab27e1debadf4706236b8b9e8415c3`.

The direct Command model/thinking evidence patch is installed as Asael 1.21.1
build 33 from `Asael-1.21.1-33-macOS.dmg`, SHA-256
`c3fbf35f61b3361e8d1c287056c738ab3a7e93fba62575fe4194b677c1e58245`;
the installed app CDHash is `9f5a757f44abcba280526f83954a0b3b9d764656`.
A true cold launch exposed the validated Settings route, accepted
`gpt-6-astra` with Ultra, rendered `MODEL_ULTRA_LIVE_OK` exactly, and projected
the effective `Ultra thinking` model receipt. The prior 1.21.0+32 app is
recoverable in Trash.

The governed scoped-selection release is installed as Asael 1.22.0 build 34
from `Asael-1.22.0-34-macOS.dmg`, SHA-256
`14cfef27f044521933c84d0183cd8049759d82937ae07d60a07ffb8aa36d42f0`;
the installed app CDHash is `0277d3ea418ffcba9305867c22bbaacdb19788e0`.
Production deployment `dpl_6ayXL7trkpGDUcYk76GL8k3UGVcG` is healthy at exact
revision `5155ca23f35517815966d7e356a30d0c7586d60c` and advertises native v28
with frozen v27. The installed app rendered `SCOPE_V28_ULTRA_OK` exactly and its
protected work log showed `openai · gpt-6-astra · Ultra thinking · 12613 tokens ·
3781ms`. Switching to **This Mac** cleared the previous choice and loaded its
validated scope; selecting Forge preserved the specialist and loaded its scope
with Automatic selection before execution. The prior 1.21.1+33
app is recoverable in Trash. This release changed no database schema or worker
protocol and required no Fly deployment.
...
installed with forced RLS and narrow lifecycle-only serving privileges. P9.4
adds persisted exact-plan approval grants for reversible risk-one and risk-two
static workflow actions. Grants bind approver, principal, plan, tool contract,
full reviewed input target, budget, and expiry; claims are atomic and idempotent,
and replanning revokes old authority. Dynamic inputs, changed targets,
irreversible actions, and risk-three actions remain per-action gated. Migration
121 is installed with forced actor RLS and narrow lifecycle-only mutation.
Vercel deployment `dpl_DtNitGtzr8sLfTuwgHZ2fhtYkJzQ` serves exact revision
`c17aba2c40166f1f09ad9beee9f463ac6bba9dfe`; the compatible Fly worker/OpenAI
gateway and Playwright gateway remain healthy. P9.5 streams actor-private
browser activity from the persisted run
ledger, retains bounded screenshots plus redacted accessibility structure in
the existing scoped Playwright session, suppresses both for sensitive-entry
actions, and serves their bytes only through authenticated owner/run routes.
The reconnecting viewer advances automatically and labels active state `Live`
...
`a0742721ebb927f6537c5e877b170f686008a3a7`; all three production health
surfaces return HTTP 200. P9.7 adds actor-owned consented profiles, signed
revision- and domain-bound gateway authority, encrypted persistent Fly storage,
restricted profile egress, direct user takeover, private credential entry, and
safe handback to the paused governed execution. Migrations 122/123 are applied;
the latter restores usable exact-actor RLS on 12 affected phase ledgers. The
production profile consent/read/revoke canary passes. Vercel deployment
`dpl_3BSWUNg6dC4wg8w573FBYokHHJnk` serves exact revision
`972e60f7722c5ccec92f720d213f535d58f89716`; both Fly services are healthy and
the browser gateway reports persistent profiles enabled. P9.8 adds a fixed,
input-aware managed-Playwright classifier before approval policy: observation,
navigation, and known tab actions remain routine and reversible, while clicks,
text/form input, selection, key, drag, dialog, unknown, and privileged operations
remain explicitly governed. Only exact reviewed reversible navigation may use a
bounded workflow grant; all consequential operations require per-action approval
and the existing effect intent/verification receipt. Untrusted page/model labels
...
return HTTP 200. P9.10 then added exact-result streaming speech under the
immutable `asael-voice:1` profile, progressive PCM playback, manual
interruption, and VAD barge-in that preserves the owned conversation and
returns directly to listening. Vercel deployment
...
P9.17 then added the reviewed credential-provider interface, exact
single-transaction scope, independent provider-signature verification,
encrypted scoped-token storage, one-time actor-private grants, append-only
claims, token destruction before processor dispatch, and reconciliation-only
handling for unknown issuance outcomes. Migration 126 is installed and Vercel
...
- [ ] **Phase 13 — macOS application:** P13.1, P13.3, P13.4, P13.5, P13.6, and P13.7 are complete for the private owner-Mac product. P13.2 is code-complete with bulk Capture, drag/drop, registered Share Extension/App Group intake, native notification actions, durable receipts/cold-launch handling, APNs token/environment handling, and governed acknowledgement. Its entitlement split makes restricted APS authority explicit and fail-closed, but one real provider-delivered APNs receipt remains externally blocked by the absent Apple Team identity/profile and APNs provider credential. P13.3 retains zero active remote-browser authority, the repaired `open_url` boundary, live Chrome canaries, zero durable screenshot/Accessibility content, and removal of the obsolete Fly browser app, machine, volume, and secrets. P13.5 provides the separately signed, approval-gated direct command helper with ephemeral output. P13.6 provides bounded rich responses, copy actions, a truthful observable work log without private reasoning, identifier-safe rich text, effective Thinking receipts, scope-correct Agent/**This Mac** catalogs, and direct-only exact Model/Thinking selection. P13.7 provides the reduced-motion-safe stateful mascot and truthful voice startup/listening/finalization/transcription feedback. Asael `1.22.0` build `34` is installed; native discovery is v28 current/v27 previous; and APS remains correctly absent from the private self-signed package. The surviving worker/OpenAI egress service remains protocol-compatible. Developer ID/notarized distribution is outside the private-install scope.

The latest installed native checkpoint is Asael 1.22.0 build 34, package
SHA-256 `14cfef27f044521933c84d0183cd8049759d82937ae07d60a07ffb8aa36d42f0`.
Migrations through v205 are installed and production discovery reports native
v28 current/v27 previous. The native Automation Studio remains available, and
the local command runner's positive and fail-closed canaries plus the rich
response/work-log, model/Thinking, and mascot/voice canaries passed in the live
app. The scoped direct, **This Mac**, and Forge catalog canaries also passed. The
prior 1.21.1+33 app is recoverable in Trash. The unchecked Phase 13
...
The source cutover removes the Playwright Fly gateway/proxy and Browser Use
...
evidence is readable history and no longer a release gate. No Playwright remains
in the repository runtime or development toolchain: its package, executable
test suite, CI job, benchmark, visual-smoke scripts, proxy, connector presets,
container, and Fly source definition are removed. Historical compatibility and
audit readers are not executable browser authority.

Native v14 retains immutable v13, removes the retired remote-frame operation, and
keeps governed Chrome `open_url`, screenshot presentation, and snapshot-bound
`screenshot_pixel` mapping. Migration 180 binds the native session to its exact
run. Installed migration 181 revokes active remote profiles/takeovers, disables
and credential-scrubs known remote-browser connectors, and preserves their tables
as read-only audit. Its checksum is
`2d8bfc80ac843fe49ca79024022b873f5046a68822892ace7ff78d393025cf4d`,
and the verified active-authority aggregate is zero. The `computer_use` assignment
