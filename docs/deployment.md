...
Native contract artifacts are committed release inputs under `public/native-contracts/v2` and `public/native-contracts/v3`; v1 remains an unadvertised archive. Run `npm run check:native-contracts` before a native-contract release; the check fails if the generated OpenAPI, event schema, fixtures, integrity manifests, Dart SDK, or frozen v2 document hashes drift. Keep v3 current and v2 supported as the previous version during this rollout. Removing an archived version requires a separately reviewed adoption decision and is not implied by a Vercel deployment.
