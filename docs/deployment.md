...
- Market research: server-only `TWELVE_DATA_API_KEY` for XAU/USD bars, `TRADERMADE_REST_API_KEY` for entitled NAS100 history, worker-only `TRADERMADE_STREAM_API_KEY` for eventual durable streaming, and `FRED_API_KEY` for official release dates and ALFRED vintages. Never expose these through `NEXT_PUBLIC_*`; TraderMade CFD availability is plan-dependent and must fail closed when the account lacks the requested dataset.
