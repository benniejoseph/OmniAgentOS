...
async function main() {
  if (!tenantId) {
    throw new Error(
      "Usage: npx tsx scripts/check-run-checkpoint-shadows.ts <tenant-id> [limit] [approval|expanded|expanded_canary]",
    );
  }
  if (
    mode !== "approval" &&
    ...
    throw new Error(
      "Checkpoint reconciliation mode must be approval, expanded, or expanded_canary.",
    );
  }
  if (!hasDatabaseUrl()) {
